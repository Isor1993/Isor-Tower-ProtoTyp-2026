...
* 0-1, index convention [x, z]) from layered Perlin noise, driven by a
* TerrainConfig asset. Pure data-in/data-out: no scene objects, no Unity
* state. Stage 2 of the terrain pipeline - MeshBuilder turns the result
* into a mesh.
*
* History :
...
{
    /// <summary>
    /// Generates a square heightmap from layered Perlin noise and
    /// reshapes it through the config's height curve.
    /// </summary>
    ...
    public static float[,] Generate(TerrainConfig config)
    {
        float[,] heightmap = new float[config.HeightmapResolution, config.HeightmapResolution];
        ...
        for (int z = 0; z < config.HeightmapResolution; z++)
        {
            for (int x = 0; x < config.HeightmapResolution; x++)
            {
                ...
                for (int o = 0; o < config.Octaves; o++)
                {
                    ...
                    float sampleX = x / config.NoiseScale * frequency + octaveOffsets[o].x;
                    float sampleZ = z / config.NoiseScale * frequency + octaveOffsets[o].y;
